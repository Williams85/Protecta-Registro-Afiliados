   ...
   Dim dtCarga As DataSet
    ...
    Protected Sub imbAceptar1_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imbAceptar1.Click
      Try
                  ...
                  Select Case ESTADO
                     ...
                     Case "C"
                        ...
                        txtCiclo2.Text = ddlCiclo.Text
                                ...
                                'Se agrego poliza pero ya no se usara esto
         ...
         Else
            AlertaScripts("Debe seleccionar CANAL, PRODUCTO y PÓLIZA para seguir con la carga.")
         End If
        ...
        Try
            If StatusSession() Then
                If Not Page.IsPostBack Then
                    ...
                    'Dim instance As New HttpRuntimeSection
                    'Dim value As Integer
                    'value = instance.MaxRequestLength
                    'instance.MaxRequestLength = 1000
    ...
    Protected Sub btnSiguiente_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSiguiente.Click
        ...
        Try
            ...
            Dim dt As DataTable
            Dim vtabla As String = dtformato.Tables(0).Rows(0).Item("Tabla").ToString
            'original ya no se usa pero le ponemos la poliza por el cambio en la bd
    ...
    Protected Sub btnSiguientePaso3_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSiguientePaso3.Click
        If HF_TIPO.Value = "M" Then
            AlertaScripts("No es posible acceder al siguiente paso por no tratarse de una carga colectiva")
            Exit Sub
        End If
      ...
      Dim RutaBat As String = ""
        ...
        Try
         ...
         If FileUpload1.PostedFile.FileName.ToString <> "" And txtCabecera.Text.Trim <> "" Then
            ...
            '            Dim sysFolder As String = RutaBat & "/bat/" & ddlCanal.SelectedValue & "POL" & ddlPoliza.SelectedValue & ".bat"
            ...
            ' Dim ourFileGenPorcia As FileInfo = New FileInfo(RutaBat & "/bat/" & ddlCanal.SelectedValue & "POL" & ddlPoliza.SelectedValue & ".bat")
            'If ourFileGenPorcia.Exists Then
            'My.Computer.FileSystem.DeleteFile(RutaBat & "/bat/" & ddlCanal.SelectedValue & "POL" & ddlPoliza.SelectedValue & ".bat")
            'End If
...
salto:
        ...
        Try
            ...
            If RadioButtonList2.Visible = True Then
                If RadioButtonList2.Items(0).Selected = False And RadioButtonList2.Items(1).Selected = False Then
                    AlertaScripts("Debe seleccionar una opción de proceso")
                    Exit Sub
                End If
            End If
            dtArchivo = Session("DATOS_ARCHIVO")
        ...
        Try
            ...
            If RadioButtonList1.Visible = True Then
                ...
                If RadioButtonList1.Items(0).Selected = True Then
                    lblMensajeProceso.Text = "Este proceso anulará toda la información activa. ¿Desea de Procesar?"
                Else
                    ...
                    If fila.Length > 0 And fila(0)(0).ToString.Trim.Length > 0 Then
                        If CDate(fila(0)(0).ToString) < CDate(txtFechaDef.Text) Then
                            lblMensajeProceso.Text = "La fecha de Expiración de la póliza es menor que la fecha de efecto ingresada. verifique"
                            Exit Sub
                        End If
                    End If
                    lblMensajeProceso.Text = "Este proceso cargará certificados nuevos y actualizará los existentes. ¿Desea Procesar?"
                End If
                mpeCarga.Show()
                'ElseIf txtFechaDef.Visible = True Then
    ...
    Protected Sub btnProcesarPaso4_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnProcesarPaso4.Click
        ...
        Try
            ...
            If IsDate(txtFechaEfecto.Text) = False Then
                AlertaScripts("Ingrese la fecha correctamente")
                Exit Sub
            End If

            dtArchivo = Session("DATOS_ARCHIVO")
            dtModulos = Session("MODULOS")
            ...
            For Each fila As DataRow In dtModulos.Select("")
                ...
                End If
                System.Threading.Thread.Sleep(500)
                'Dim proce As New Process
                'proce.StartInfo.WorkingDirectory = RutaBat & "/bat/"
                'proce.StartInfo.FileName = ddlCanal.SelectedValue & "POL" & ddlPoliza.SelectedValue & "_CC.bat"
                'proce.Start()
                'proce.WaitForExit()
                'proce.Close()
                'proce.Dispose()
            ...
            'Dim ourFileGen As FileInfo = New FileInfo(RutaBat & "/bat/" & ddlCanal.SelectedValue & "POL" & ddlPoliza.SelectedValue & "_CC.bat")
            'If ourFileGen.Exists Then
            '    My.Computer.FileSystem.DeleteFile(RutaBat & "/bat/" & ddlCanal.SelectedValue & "POL" & ddlPoliza.SelectedValue & "_CC.bat")
            'End If
            ...
            AlertaScripts("Carga Completada")
    ...
    Private Sub CorreProcesoColectiva()
        ...
        Dim RutaBat As String = Server.MapPath("").Replace("\", "/")
        pro.StartInfo.WorkingDirectory = RutaBat & "/bat/"
        'pro.StartInfo.WorkingDirectory = HF_RUTA_CARGA.Value
...
salto:
    ...
    Private Sub CreaBAT_Colectiva(ByVal Periodo As String, ByVal SecPol As String)
        Try
            ...
            Dim s As New System.Text.StringBuilder
            s.AppendLine("@echo off")
            's.AppendLine("NET USE Z: " & HF_SERVER.Value & "\CargaMasiva " & HF_PASSINT.Value & " /USER:" & HF_SERVER.Value & "\IUSR_INTEGRA /Y")
            ...
            's.AppendLine("NET USE W: " & HF_SERVER.Value & "\CargaMasiva /delete ")
            's.AppendLine("pause")
            tex = s.ToString
            ...
            'theFile = File.Open(HF_RUTA_CARGA.Value & ddlCanal.SelectedValue & "POL" & ddlPoliza.SelectedValue & "_CC.bat", FileMode.OpenOrCreate, FileAccess.Write)
            Dim filaEscribe As StreamWriter = New StreamWriter(theFile)
   ...
   Protected Sub ddlPoliza_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlPoliza.SelectedIndexChanged
      ...
      Try
         ...
         If ddlProducto.SelectedValue <> "" And ddlPoliza.SelectedValue <> "" Then
            ...
            ' If dtCarga.Tables(0).Rows.Count > 0 And dtCarga.Tables(1).Rows.Count > 0 Then
   ...
   End Sub
    ...
    Private Sub CreaArchivoCTL_Paso4(ByVal modulo As Integer)
        ...
        If vTipoCarga = "C" Then
            ...
            If vCargaBeneficiario = "S" Then
                ...
                str.AppendLine("  SVALUESLIST )")
            End If
            'If vCargaIntermediario = "S" Then
            '    str.AppendLine("INTO TABLE ""T_MASIVECHARGE"" WHEN tab='2'")
            '    str.AppendLine("FIELDS TERMINATED BY "","" OPTIONALLY ENCLOSED BY '" & caracter2 & "' TRAILING NULLCOLS")
            '    str.AppendLine("( tab FILLER POSITION (1:1) CHAR(1) ,")
            '    str.AppendLine("  coma filler char(1) ,")
            '    str.AppendLine("  SKEY  CONSTANT '" & vSKEY3 & "',")
            '    str.AppendLine("  NROWS ,")
            '    str.AppendLine("  NCOLUMNS ,")
            '    str.AppendLine("  SFIELD ,")
            '    str.AppendLine("  SVALUE ,")
    ...
    Function ValidaExpresion(ByVal patron As String, ByVal dato As String) As Boolean
        ...
        Catch ex As Exception
            Throw ex
        End Try
        'If Me.ValidaExpresion("[0-9]", Me.txtAñoDiferido.Text) = False Then
    End Function
    ...
    Protected Sub btnRegCorrectos_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnRegCorrectos.Click
        ...
        Try
            ...
            dt = Metodos.Lista_Report_Correctos(Metodos.DB.ORACLE, vtabla)
    ...
    Protected Sub btnRptAgrobanco_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnRptAgrobanco.Click
        Try
            ...
            dt = Metodos.Lista_Report_Agrobanco(Metodos.DB.ORACLE, sNroArchivo)
    ...
    Private Sub Remplazo_Nomina_90()
        Try
            ...
            If DT_FECHAS.Rows(0).Item(2).ToString <> "" Then
               ...
               If RadioButtonList1.Visible = True And RadioButtonList1.Items(1).Selected = True Then
                  If CDate(txtFechaDef.Text) < CDate(DT_FECHAS.Rows(0).Item(0).ToString) Then
                     AlertaScripts("No puede colocar una fecha menor a la cargada por el sistema")
                     Exit Sub
                  End If
               ElseIf RadioButtonList1.Visible = True And RadioButtonList1.Items(0).Selected = True Then
                  If CDate(txtFechaDef.Text) <= CDate(DT_FECHAS.Rows(0).Item(0).ToString) Then
                     AlertaScripts("No puede colocar una fecha menor o igual a la cargada por el sistema")
                     Exit Sub
                  End If
               End If
               If DT_FECHAS.Rows(0).Item(2).ToString <> "6" Then
                  If CDate(txtFechaDef.Text) > CDate(DT_FECHAS.Rows(0).Item(1).ToString) Then
                     AlertaScripts("No puede colocar una fecha mayor a la fecha de proxima emisión de recibo")
                     Exit Sub
                  End If
               End If
            End If
         ...
         Dim dia As String = txtFechaDef.Text.Substring(0, 2)
            Metodos.Archivo_Genera_Tarea90(Metodos.DB.ORACLE, ddlPoliza.SelectedValue, Session("CodUsuario"), año & mes & dia, vTransformada, IIf(RadioButtonList1.Items(0).Selected = True, "R", "C"))
