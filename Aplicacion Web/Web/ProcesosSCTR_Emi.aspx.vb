...
Partial Class ProcesosSCTR_Emi
...
#Region "Combo"
    ...
    Private Sub ObtenerFormato()
        dSource = Metodos.List_Param_Formato_SCTR(Metodos.DB.ORACLE, 120)
...
salto:
        ...
        If boolStatus <> True Then
            ...
            ERROR_ = ConfigurationManager.AppSettings("FormatoSCTREmiMatriz").ToString()
            sArchivo = Metodos.CloseLoadFileSctr(Metodos.DB.ORACLE, ERROR_, Convert.ToInt32(hdIDArchivoProcesador.Value))
        End If
...
#End Region
    ...
    Protected Sub btnValidar_Click(sender As Object, e As EventArgs) Handles btnValidar.Click
        ...
        ObtenerFormato()
        Try
            'FileUpload1.FileName = "Hola"

            Dim FileName As String = FileUpload1.FileName.ToString()
            If txtCabecera.Text <> "" And FileName <> "" Then
                HiddenField2.Value = FileName
                lblruta.Text = FileName
                dtformato = Metodos.List_Format_SCTR_Masiva(Metodos.DB.ORACLE, dCodFormato)
                ...
                Dim sRutaDestino As String = LTrim(RTrim(HF_RUTA_MASIVA.Value & "MATRIZ MASIVO" & "/" & ddlCanal.Text & "/" & dFecha & "/" & Session("Nombre") & "/" & HiddenField2.Value))
                Dim sRutaTemp As String = LTrim(RTrim(HF_RUTA_MASIVA.Value & "MATRIZ MASIVO" & "/" & ddlCanal.Text & "/" & dFecha & "/" & Session("Nombre")))
                ...
                sArchivo = Metodos.OpenLoadFileSctr(Metodos.DB.ORACLE, ddlCanal.SelectedValue, sArchivoOrigen, Convert.ToInt32(txtCabecera.Text), sRutaDestino, txtComentario.Text, Convert.ToInt32(Session("CodUsuario")))
            ...
            Else
                ...
                FileUpload1.Enabled = True
                If String.IsNullOrEmpty(txtCabecera.Text) And String.IsNullOrEmpty(FileUpload1.FileName.ToString()) Then
                    AlertaScripts("Ingresar el Documento a Procesar e indicar el número de Cabecera.")
...
error_salto_1:
            ...
            If dtTable.Rows.Count = 0 Then
                ...
                If (dtTableError.Rows.Count > 0) Then
                    Session("ErrorCase") = 2
                    Dim bool As Boolean = Metodos.Ins_ErrorTrama(Metodos.DB.ORACLE, dtTableError, hdIDArchivoProcesador.Value)
                    btnError.Enabled = True
            ...
            Else
                ...
                For Each row As DataRow In dtTable.Rows
                    ...
                    sExistsPoliza = Metodos.ExistPoliza(Metodos.DB.ORACLE, nPolicy)
                ...
                If (dtTableProcesar.Rows.Count > 0) Then
                    status = True
                    Dim bool_ As Boolean = Metodos.Ins_Trama(Metodos.DB.ORACLE, dtTableProcesar, hdIDArchivoProcesador.Value)
                End If
                If (dtTableError.Rows.Count > 0) Then
                    Session("ErrorCase") = 2
                    Dim bool As Boolean = Metodos.Ins_ErrorTrama(Metodos.DB.ORACLE, dtTableError, hdIDArchivoProcesador.Value)
            ...
            Try
                contador_respuesta = Metodos.ValidateTrama(Metodos.DB.ORACLE, hdIDArchivoProcesador.Value)
            Catch ex As Exception
...
#Region "Utilitarios"
    ...
    Private Function EvalColumn(ByVal InDicador As Integer, ByVal Campo As String) As Integer
        ...
        Select Case dColumna.Rows(InDicador)("CAMPO_REQUERIDO")
            ...
            Case "S"
                ...
                'If dColumna.Rows(InDicador)("TIPODATO") = "N" Then
                '    If Campo.Length = 0 Then
                '        accion = 0
                '    Else
                '        If Campo.Length <= Convert.ToInt32(dColumna.Rows(InDicador)("LONGITUD")) Then
    ...
    Private Function GetErrorCase() As DataTable
        ...
        Try
            ...
            dtResult = Metodos.List_Error_SCTR_Masiva(Metodos.DB.ORACLE, Convert.ToInt32(hdIDArchivoProcesador.Value))
    ...
    Private Function GetResumen() As DataTable
        ...
        Try
            ...
            dtResult = Metodos.List_Emision(Metodos.DB.ORACLE, Convert.ToInt32(hdIDArchivoProcesador.Value))
    ...
    Private Sub CargarGrilla()
        ...
        Try
            ...
            dts = Metodos.ListGridModul(Metodos.DB.ORACLE, Getfecha())
...
#End Region
        ...
        Try
            ...
            If btnValidar.Enabled = True Then
                If (idejcucion.Value = 1) Then
                    sArchivo = Metodos.CloseLoadFileSctr(Metodos.DB.ORACLE, "1", Convert.ToInt32(hdIDArchivoProcesador.Value))
                End If
    ...
    Protected Sub btnProcesar_Click(sender As Object, e As EventArgs) Handles btnProcesar.Click
        Try
            ...
            Dim dtResult As New DataSet
            Dim sArchivo As Integer = 0
            idrespuesa = Metodos.Ins_Procesar_core(Metodos.DB.ORACLE, Convert.ToInt32(hdIDArchivoProcesador.Value))
            dtResult = Metodos.List_Error_SCTR_Masiva(Metodos.DB.ORACLE, Convert.ToInt32(hdIDArchivoProcesador.Value))


            If idrespuesa = 0 Then
                Session("ErrorCase") = 3
                sArchivo = Metodos.CloseLoadFileSctr(Metodos.DB.ORACLE, "2", Convert.ToInt32(hdIDArchivoProcesador.Value))
                btnProcesar.Enabled = True
            Else
                sArchivo = Metodos.CloseLoadFileSctr(Metodos.DB.ORACLE, "Se proceso la trama.", Convert.ToInt32(hdIDArchivoProcesador.Value))
            End If
